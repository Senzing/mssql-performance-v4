...
```sql
ALTER DATABASE G2 SET RECOVERY SIMPLE;
```
...
```sql
ALTER DATABASE G2 SET QUERY_STORE = OFF;
```
...
Verified on the Sayari 1B fleet at 500: every engine statement runs at **average DOP 1.00**, and the only
...
on a monotonic ID, which is most of the hot set. Measured on a 16.4-billion-row Sayari load:
...
```sql
ALTER DATABASE G2 MODIFY FILE (NAME = G2, SIZE = <presize>, FILEGROWTH = 32768MB);
ALTER DATABASE G2 ADD FILE (NAME = G2_2, FILENAME = '.../G2_2.ndf', SIZE = <presize>, FILEGROWTH = 32768MB)
  TO FILEGROUP [PRIMARY];
-- ...repeat to ~8 equally-sized files...
ALTER DATABASE G2 MODIFY FILEGROUP [PRIMARY] AUTOGROW_ALL_FILES;
```
